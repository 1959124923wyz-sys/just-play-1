    ...
    if locked:
        ...
        for choice, missing_flags, missing_stats in locked:
            ...
            if missing_flags:
                ...
                requirements.append(f"你尚未掌握：{', '.join(flag_texts)}")
            if missing_stats:
                stat_texts = []
                for key, value in missing_stats:
                    if key == "silver":
                        stat_texts.append("银钱不够")
                    elif key == "suspicion":
                        stat_texts.append("你察觉有人盯上了你")
                    else:
                        stat_texts.append(f"{key}未达要求")
...
def step(state: dict, user_input: str, story: dict):
    ...
    if trimmed == "0":
        ...
        output_lines.append("提示：多探索可解锁隐藏路径。")
        if help_mode == 1:
            ...
            if flags:
                flag_descriptions = story.get("flag_descriptions", {})
                output_lines.append("幕后线索：")
                for flag in sorted(flags):
                    output_lines.append(f"- {flag_descriptions.get(flag, flag)}")
            output_lines.append(
                f"幕后数值：嫌疑={stats.get('suspicion', 0)} 银两={stats.get('silver', 0)}"
            )
        ...
        if 1 <= choice_index <= len(choices):
            ...
            for key, delta in delta_stats.items():
                ...
                if delta != 0:
                    if key == "silver":
                        if delta > 0:
                            output_lines.append("你顺手收了些银钱，心里一沉。")
                        else:
                            output_lines.append("你付出了银钱，换来一时通行。")
                    elif key == "suspicion":
                        if delta > 0:
                            output_lines.append("你感觉目光变得更紧，似有人留心。")
                        else:
                            output_lines.append("周围的戒备似乎稍缓。")
            ...
            if stats_config:
                ...
                if suspicion_max is not None and fail_node and stats.get("suspicion", 0) >= suspicion_max:
                    ...
                    output_lines.append("你的举动引来更多注视，局势骤然收紧。")
